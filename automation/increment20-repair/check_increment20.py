...
def check_repository(root: Path) -> list[Problem]:
    ...
    for relative in TEMPORARY_FILES:
        if (root / relative).exists():
            problems.append(
                Problem("NODAL-INC20-002", f"temporary file remains: {relative}")
            )
    ...
    require(
        ...
        (
            ...
            '"nodal.module"',
            "nodal.bridge.interface_abi",
            "nodal.bridge.resolved_nets",
            "nodal.bridge.topology",
            "parameter_bindings",
            "source_end_line",
            "loc(",
            "SHA-256",
    ...
    if "Runtime.getRuntime.exec" in process or "bash -c" in process:
        problems.append(
            Problem("NODAL-INC20-004", "process implementation uses a shell command")
        )
    require(
        tests,
        (
            "deterministic source-correlated textual MLIR",
            "snapshot insertion order does not affect the bridge",
            "argv-safe process success and nonzero failure",
            "timeout is distinct and cleanup is transactional",
            "locked nodalc parses bridge MLIR when configured",
        ),
        problems,
        "NODAL-INC20-005",
        "Scala bridge tests",
    ...
    if status == "implemented-awaiting-evidence":
        if not unchecked or revision < (1, 23):
            problems.append(
                Problem(
                    "NODAL-INC20-008",
                    "pre-evidence state must leave Increment 20 unchecked",
                )
            )
    ...
    if not increment21_unchecked:
        problems.append(Problem("NODAL-INC20-008", "Increment 21 must remain unchecked"))

...
scripts=(
  inc34_patch_scala.py
  inc34_patch_native.py
  inc34_patch_contracts.py
  inc34_post_repair.py
  inc34_patch_native_dataflow.py
  inc34_patch_native_dataflow_repair.py
  inc34_patch_native_dataflow_portability.py
  inc34_patch_native_dataflow_contract_repair.py
)
script_paths=()
for script in "${scripts[@]}"; do
  script_paths+=("${controller}/.github/automation/${script}")
done

gate scripts-compile python3 -m py_compile "${script_paths[@]}"
if [[ ${failed} -eq 0 ]]; then
  for script in "${scripts[@]}"; do
    gate "apply-${script%.py}" python3 \
      "${controller}/.github/automation/${script}" --root "${work}"
  done
fi
...
if [[ -d ${RUNNER_TEMP}/nodal-lint-toolchain ]]; then
  gate scala-format bash -lc \
    "cd '${work}' && ./mill mill.scalalib.scalafmt.ScalafmtModule/reformatAll __.sources"
...
gate runtime-witness bash -lc \
  "cd '${work}' && output='${RUNNER_TEMP}/increment34-runtime.txt' && ./mill -i examples.continuousTimeApi.runMain nodal.increment34fixture.Increment34RuntimeCheck \"\${output}\" && grep -F 'conditional_definite=true' \"\${output}\" && grep -F 'case_definite=true' \"\${output}\" && grep -F 'loop_definite=true' \"\${output}\" && grep -F 'static_definite=true' \"\${output}\""
...
if [[ -d ${work}/out ]]; then
  gate direct-native-dataflow bash -lc \
    "cd '${work}' && nodalc=\$(find out -type f -name nodalc -perm -u+x | head -n 1) && test -n \"\${nodalc}\" && \"\${nodalc}\" --mlir-print-op-generic --mlir-print-debuginfo core/compiler/test/IR/analog-control-flow.mlir > '${RUNNER_TEMP}/increment34-structured.mlir' && grep -F '\"nodal.analog_if\"' '${RUNNER_TEMP}/increment34-structured.mlir' && grep -F '\"nodal.analog_case\"' '${RUNNER_TEMP}/increment34-structured.mlir' && grep -F '\"nodal.analog_loop\"' '${RUNNER_TEMP}/increment34-structured.mlir' && grep -F 'ControlTop.final_read' '${RUNNER_TEMP}/increment34-structured.mlir' && grep -F 'AnalogControlFlow.scala' '${RUNNER_TEMP}/increment34-structured.mlir' && for fixture in missing-else missing-default zero-trip continue-path; do set +e; \"\${nodalc}\" \"core/compiler/test/IR/analog-control-flow-invalid-\${fixture}.mlir\" >'${RUNNER_TEMP}/'\"\${fixture}\"'.out' 2>'${RUNNER_TEMP}/'\"\${fixture}\"'.err'; rc=\$?; set -e; test \"\${rc}\" -ne 0; grep -F 'NODAL-ANALOG-034-004' '${RUNNER_TEMP}/'\"\${fixture}\"'.err'; done"
else
  printf '%s\tSKIP\t1\n' direct-native-dataflow >> "${status}"
  failed=1
fi
...
cp "${RUNNER_TEMP}/increment34-runtime.txt" "${report}/" 2>/dev/null || true
cp "${RUNNER_TEMP}/increment34-construction.txt" "${report}/" 2>/dev/null || true
cp "${RUNNER_TEMP}/increment34-structured.mlir" "${report}/" 2>/dev/null || true
cp "${RUNNER_TEMP}"/*.err "${report}/" 2>/dev/null || true
...
if [[ ${failed} -eq 0 ]]; then
  ...
  if git -C "${work}" diff --cached --quiet; then
    ...
    git -C "${work}" commit -m \
      'feat(increment34): complete structured native dataflow'
    published=$(git -C "${work}" rev-parse HEAD)
...
{
  echo '# Increment 34 native dataflow v6'
  echo
  echo '```text'
  ...
  echo
  if [[ ${failed} -eq 0 ]]; then
    echo "**PASS — published exact head \`${published}\` to PR #109.**"
  else
    echo '**FAIL — PR #109 was not modified. See the per-gate logs in this branch.**'
  fi
} > "${report}/README.md"

result_branch=automation/inc34-native-dataflow-results-v6
rm -rf result
...
git -C result commit -m \
  "ci(increment34): record native dataflow v6 ${GITHUB_RUN_ID}"
git -C result push --force origin HEAD:"${result_branch}"

if [[ ${failed} -eq 0 ]]; then
  gh pr comment 109 --repo "${GITHUB_REPOSITORY}" \
    --body "Increment 34 structured native dataflow v6 passed every staged gate. Exact published head: \`${published}\`. Diagnostic record: branch \`${result_branch}\`, run \`${GITHUB_RUN_ID}\`."
  exit 0
fi

gh pr comment 109 --repo "${GITHUB_REPOSITORY}" \
  --body "Increment 34 structured native dataflow v6 did not publish because at least one staged gate failed. Exact logs and candidate patch: branch \`${result_branch}\`, run \`${GITHUB_RUN_ID}\`."
exit 1

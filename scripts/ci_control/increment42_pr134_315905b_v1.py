"""Dispatch only the two pinned Increment 42 conservative-connection checks.
...
HEAD = "315905bde74f23997ce1c03fe5bc3a7569a00b0d"
TREE = "e442ef59bfd257a249d83feb48ce3c6f8dfc14c2"
PARENT = "137170547bdd68ad8388c36a31b95df86f98b3fa"
CONTROL = "ci-control/nodal-42-pr134-315905b-connect-v1"
CONTROL_WORKFLOW = ".github/workflows/ci-control-42-pr134-315905b.yml"
INVENTORY = Path("scripts/ci_control/increment42_pr134_315905b_inventory.json")
INVENTORY_SHA256 = "b9aa5848121ea085f50a75171d769ee14a4a74a9632e28ac5ffa9a1bb574a5b0"
WORKFLOWS = (
...
PARENT_SUCCESSES = [{'run_id': 36221928902,
  'workflow_id': 352989730,
  'path': '.github/workflows/increment-41-analog-functions.yml',
  'run_attempt': 1,
  'jobs': [{'id': 108348684566, 'name': 'functions'}]},
 {'run_id': 36221918346,
  'workflow_id': 351939507,
  'path': '.github/workflows/increment-38-mathematical-functions.yml',
  'run_attempt': 1,
  'jobs': [{'id': 108348657952, 'name': 'functions'}]},
 {'run_id': 36221909413,
  'workflow_id': 350704824,
  'path': '.github/workflows/increment-36-time-waveform-operators.yml',
  'run_attempt': 1,
  'jobs': [{'id': 108348632812, 'name': 'increment-36-time-waveform-operators'}]},
 {'run_id': 36221905533,
  'workflow_id': 344342524,
  'path': '.github/workflows/increment-29-parameters-units.yml',
  'run_attempt': 1,
  'jobs': [{'id': 108348621072, 'name': 'increment-29/parameters-units'}]},
 {'run_id': 36221896697,
  'workflow_id': 338626156,
  'path': '.github/workflows/ci.yml',
  'run_attempt': 1,
  'jobs': [{'id': 108348595510, 'name': 'scala'},
           {'id': 108348595602, 'name': 'contracts'},
           {'id': 108348595624, 'name': 'native'},
           {'id': 108352017934, 'name': 'required'}]}]
...
def request(path, payload=None):
    ...
    req = urllib.request.Request(API + path, data=data, headers={
        ...
        "User-Agent": "Nodal-42-PR134-315905b-connect-v1",
...
def validate_parent_successes():
    # A launch sequencing guard only. Old-head checks never qualify new source.
    for expected in PARENT_SUCCESSES:
        ...
        require(run["id"] == number and run["workflow_id"] == expected["workflow_id"]
                ...
                "parent success run identity/outcome changed")
        jobs = []
        for page in range(1, 21):
            ...
            raise RuntimeError("parent success job pagination bound reached")
        require(len(jobs) == len(expected["jobs"])
                and {(job["id"], job["name"]) for job in jobs}
                == {(job["id"], job["name"]) for job in expected["jobs"]},
                "parent success latest-attempt job set changed")
        require(all(job["run_id"] == number and job["run_attempt"] == expected["run_attempt"]
                    and job["head_sha"] == PARENT and job["status"] == "completed"
                    and job["conclusion"] == "success" for job in jobs),
                "parent success job identity/outcome changed")
        record("parent-success-identity-verified-not-new-head-qualification",
               run=number, jobs=[job["id"] for job in jobs], candidate=PARENT)

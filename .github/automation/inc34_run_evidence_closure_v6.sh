...
patched_runner=${RUNNER_TEMP}/inc34_run_evidence_closure_v6_patched.sh
...
followup4=${controller}/.github/automation/inc34_evidence_closure_followup_v4.py
followup5=${controller}/.github/automation/inc34_evidence_closure_followup_v5.py

python3 - "${source_runner}" "${patched_runner}" \
  "${followup1}" "${followup2}" "${followup3}" "${followup4}" "${followup5}" <<'PY'
...
if anchor not in text:
    raise SystemExit("closure runner v6 injection anchor was not found")

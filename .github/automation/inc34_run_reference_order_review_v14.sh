...
if [[ ${feature_base} != "${remote_feature}" ]]; then
  echo 'feature branch moved before final reference review' >&2
  exit 1
fi

gate predecessor-ancestry bash -lc \
  "cd '${work}' && git merge-base --is-ancestor origin/dev HEAD"
gate prerequisite-v12-v13 bash -lc \
  "cd '${work}' && grep -F 'native_unreachable_structural_reference_validation' tests/compiler/fixtures/increment34/manifest.json && grep -F 'canonical_control_owner_identity' tests/compiler/fixtures/increment34/manifest.json && grep -F 'pre_control_lexical_scope_alignment' tests/compiler/fixtures/increment34/manifest.json"

patcher=${controller}/.github/automation/inc34_reference_order_review_v14.py
gate patcher-compile python3 -m py_compile "${patcher}"
if [[ ${failed} -eq 0 ]]; then
  gate apply-reference-order-review python3 "${patcher}" --root "${work}"
fi
...
gate runtime-witness bash -lc \
  "cd '${work}' && output='${RUNNER_TEMP}/increment34-runtime-v14.txt' && ./mill -i examples.continuousTimeApi.runMain nodal.increment34fixture.Increment34RuntimeCheck \"\${output}\" && grep -F 'unreachable_unknown=NODAL-ANALOG-034-014' \"\${output}\" && grep -F 'escaped_local=NODAL-ANALOG-034-014' \"\${output}\" && grep -F 'forward_reference=NODAL-ANALOG-034-014' \"\${output}\""
gate construction-witness bash -lc \
  "cd '${work}' && output='${RUNNER_TEMP}/increment34-construction-v14.txt' && ./mill -i examples.continuousTimeApi.runMain nodal.increment34fixture.Increment34ConstructionCheck \"\${output}\" && grep -F 'precontrol_scope_aligned=true' \"\${output}\" && grep -F 'empty_owner=NODAL-ANALOG-034-001' \"\${output}\" && grep -F 'padded_owner=NODAL-ANALOG-034-001' \"\${output}\""
...
if [[ -d ${RUNNER_TEMP}/nodal-native-toolchain && \
  ...
  if [[ -n ${nodalc} ]]; then
    ...
    gate direct-forward-reference-diagnostic bash -lc \
      "cd '${work}' && set +e; '${nodalc}' core/compiler/test/IR/analog-control-flow-invalid-forward-reference.mlir > '${report}/direct-forward-reference.out' 2>&1; rc=\$?; set -e; test \${rc} -ne 0; grep -F 'NODAL-ANALOG-034-014' '${report}/direct-forward-reference.out'"
    gate direct-owner-diagnostic bash -lc \
      "cd '${work}' && set +e; '${nodalc}' core/compiler/test/IR/analog-control-flow-invalid-owner.mlir > '${report}/direct-owner.out' 2>&1; rc=\$?; set -e; test \${rc} -ne 0; grep -F 'NODAL-ANALOG-033-009' '${report}/direct-owner.out'"
  else
    printf '%s\tFAIL\t1\n' direct-unreachable-reference-diagnostic >> "${status}"
    printf '%s\tFAIL\t1\n' direct-forward-reference-diagnostic >> "${status}"
    printf '%s\tFAIL\t1\n' direct-owner-diagnostic >> "${status}"
    failed=1
  fi
...
if [[ ${failed} -eq 0 ]]; then
  ...
  if ! git -C "${work}" diff --cached --quiet; then
    git -C "${work}" commit -m \
      'fix(increment34): enforce reference visibility and declaration order'
  fi
...
{
  echo '# Increment 34 reference-order review v14'
  echo
  echo '```text'
...
result_branch=automation/inc34-reference-order-results-v14
rm -rf result
...
git -C result commit -m \
  "ci(increment34): record reference-order review v14 ${GITHUB_RUN_ID}"
git -C result push --force origin HEAD:"${result_branch}"

if [[ ${failed} -eq 0 ]]; then
  gh pr comment 109 --repo "${GITHUB_REPOSITORY}" \
    --body "Increment 34 final reference-order review passed. Exact published head: \`${published}\`; run: \`${GITHUB_RUN_ID}\`."
  exit 0
fi

gh pr comment 109 --repo "${GITHUB_REPOSITORY}" \
  --body "Increment 34 reference-order review did not publish because a staged gate failed. Logs: branch \`${result_branch}\`, run \`${GITHUB_RUN_ID}\`."
exit 1

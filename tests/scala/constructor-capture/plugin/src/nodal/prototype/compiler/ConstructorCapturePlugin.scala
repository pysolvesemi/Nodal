...
/** Compile-only probe, deliberately restricted to literal Real defaults. No production Nodal
  * classes are linked into the compiler plugin.
  */
...
private final class ConstructorCapturePhase extends PluginPhase:
  ...
  /** Scan every already-typed unit before transforming any one unit. DefTree is not generally
    * retained for non-inline default getters; traversing the real units also handles a constructor
    * definition later in the source list.
    */
  ...
  private def capture(
    ...
    if !plain then
      ...
      fail(
        "SHAPE",
        "partial probe requires a named top-level direct Module subclass, " +
          "one primary parameter list and no type parameters or secondary constructors",
        definition
      )
        ...
        then
          ...
          literal match
            ...
            case _ =>
              ...
              fail(
                "DEFAULT",
                "partial probe accepts only a finite Double literal default " +
  ...
  /** This symbol equality admits only the known pure literal lifting method. It does not execute
    * the getter or inspect arbitrary host expression names.
    */
  ...
  /** Keep the compiler's original getter call and all named-argument temporaries. Change only its
    * approved pure literal body into an inert omitted marker. Explicit arguments execute no getter;
    * omitted arguments execute it once.
    */
        ...
        annotations match
          case annotation :: Nil if annotation.arguments.size == 2 =>
            ...
            if !versionOne || encoded.isEmpty then
              ...
              try
                val parameters = if encoded.get.isEmpty then Nil
                else

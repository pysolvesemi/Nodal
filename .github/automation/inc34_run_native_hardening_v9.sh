...
patcher=${controller}/.github/automation/inc34_native_hardening_v9.py
gate patcher-compile python3 -m py_compile "${patcher}"
if [[ ${failed} -eq 0 ]]; then
  gate apply-hardening python3 "${patcher}" --root "${work}"
fi
...
if [[ -d ${work}/out ]]; then
  gate direct-hardening-diagnostics bash -lc \
    "cd '${work}' && nodalc=\$(find out -type f -name nodalc -perm -u+x | head -n 1) && test -n \"\${nodalc}\" && for item in duplicate-identity:001 order:014 guard-read:004 case-label:007; do fixture=\${item%%:*}; code=\${item##*:}; set +e; \"\${nodalc}\" \"core/compiler/test/IR/analog-control-flow-invalid-\${fixture}.mlir\" >'${RUNNER_TEMP}/'\"\${fixture}\"'.out' 2>'${RUNNER_TEMP}/'\"\${fixture}\"'.err'; rc=\$?; set -e; test \"\${rc}\" -ne 0; grep -F \"NODAL-ANALOG-034-\${code}\" '${RUNNER_TEMP}/'\"\${fixture}\"'.err'; done"
else
  printf '%s\tSKIP\t1\n' direct-hardening-diagnostics >> "${status}"
  failed=1
fi
...
if [[ ${failed} -eq 0 ]]; then
  ...
  if git -C "${work}" diff --cached --quiet; then
    ...
    git -C "${work}" commit -m \
      'fix(increment34): harden native control-flow verification'
    published=$(git -C "${work}" rev-parse HEAD)
...
{
  echo '# Increment 34 native hardening v9'
  echo
  echo '```text'
  ...
  echo
  if [[ ${failed} -eq 0 ]]; then
    echo "**PASS — published exact hardening head \`${published}\` to PR #109.**"
  else
    echo '**FAIL — PR #109 was not modified. See the per-gate logs in this branch.**'
  fi
} > "${report}/README.md"

result_branch=automation/inc34-native-hardening-results-v9
rm -rf result
...
git -C result commit -m \
  "ci(increment34): record native hardening v9 ${GITHUB_RUN_ID}"
git -C result push --force origin HEAD:"${result_branch}"

if [[ ${failed} -eq 0 ]]; then
  gh pr comment 109 --repo "${GITHUB_REPOSITORY}" \
    --body "Increment 34 native verifier hardening v9 passed every staged gate. Exact published head: \`${published}\`. Diagnostic record: branch \`${result_branch}\`, run \`${GITHUB_RUN_ID}\`."
  exit 0
fi

gh pr comment 109 --repo "${GITHUB_REPOSITORY}" \
  --body "Increment 34 native verifier hardening v9 did not publish because at least one staged gate failed. Exact logs and candidate patch: branch \`${result_branch}\`, run \`${GITHUB_RUN_ID}\`."
exit 1

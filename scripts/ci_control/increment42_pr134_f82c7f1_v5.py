"""Dispatch only exact-head Increment 42 checks after the pinned formatting repair.

This controller is a bounded CI dispatcher. It never executes feature source,
publishes source, merges, cancels work, or changes repository protections.
A green controller is only a dispatch receipt; dispatched jobs are qualification.
"""
...
HEAD = "f82c7f1cf5c30631d6d5bf6f46f2088fa0f0341d"
TREE = "026ebe9432ab6698a473ce123e1fbca9140b27c5"
PARENT = "9bd5db950c7e103bf86807276f7dc0814b1a735b"
CONTROL = "ci-control/nodal-42-pr134-f82c7f1-targeted-v5"
SOURCE_PATH = "core/compiler/lib/Transforms/Passes.cpp"
SOURCE_SHA256 = "c5a4e54be5ac8c517fe230d6c98f68cc2e19f4c66759a2d7c7246b51b5933a51"
WORKFLOWS = (
...
FAILED_RUNS = (
    (35857009774, 338626156, "core"),
    (35857023864, 342183625, "increment21"),
    (35857037858, 342322970, "increment22"),
    (35857051476, 342865392, "increment23"),
)
PARENT_SUCCESS = (35857065536, 352989730)
...
def request(path, payload=None):
    ...
    if payload is not None:
        ...
        require(path.startswith(("/git/ref/heads/", "/git/commits/", "/pulls/",
                                 "/contents/", "/actions/workflows/", "/actions/runs/")),
    ...
    req = urllib.request.Request(API + path, data=data, headers={
        ...
        "User-Agent": "Nodal-42-PR134-f82c7f1-targeted-v5",
...
def validate_parent_evidence():
    for run_id, workflow_id, label in FAILED_RUNS:
        run = request(f"/actions/runs/{run_id}")
        require(run["id"] == run_id and run["workflow_id"] == workflow_id,
                f"{label} parent run identity mismatch")
        require(run["head_sha"] == PARENT and run["head_branch"] == REF
                and run["event"] == "workflow_dispatch"
                and run["status"] == "completed" and run["conclusion"] == "failure",
                f"{label} parent failure evidence changed")
        jobs = request(f"/actions/runs/{run_id}/jobs?filter=latest&per_page=100")["jobs"]
        require(jobs, f"{label} parent failure has no jobs")
        by_name = {job["name"]: job["conclusion"] for job in jobs}
        if label == "core":
            require(by_name.get("native") == "success" and by_name.get("scala") == "success"
                    and by_name.get("contracts") == "failure"
                    and by_name.get("required") == "failure",
                    "Core parent failure shape changed")
        else:
            require(len(jobs) == 1 and jobs[0]["conclusion"] == "failure",
                    f"{label} parent failure shape changed")
    run_id, workflow_id = PARENT_SUCCESS
    run = request(f"/actions/runs/{run_id}")
    require(run["id"] == run_id and run["workflow_id"] == workflow_id
            and run["head_sha"] == PARENT and run["head_branch"] == REF
            and run["event"] == "workflow_dispatch"
            and run["status"] == "completed" and run["conclusion"] == "success",
            "parent Increment 41 evidence run changed")
...
def validate(definitions=True):
    ...
    require(pr["base"]["ref"] == BASE_REF and pr["head"]["ref"] == REF
            and pr["head"]["sha"] == HEAD,
            "PR ref/head mismatch")
    require(pr["base"]["repo"]["full_name"] == REPO
            and pr["head"]["repo"]["full_name"] == REPO,
            "PR repository mismatch")
    source = request("/contents/" + SOURCE_PATH + "?ref=" + HEAD)
    require(source["encoding"] == "base64", "unexpected source encoding")
    source_bytes = base64.b64decode(source["content"])
    require(hashlib.sha256(source_bytes).hexdigest() == SOURCE_SHA256,
            "formatted source bytes changed")
    if not definitions:
        return
    ...
    for page in range(1, 21):
        ...
        for run in rows:
            ...
            if run["head_sha"] == HEAD and run["head_branch"] == REF:
                ...
                raise RuntimeError("other-head run is active; do not create competing CI")
        if len(rows) < 100:
            return found
...
def dispatch_one(number, path, digest):
    ...
    if existing:
        ...
        require(all(r["status"] != "completed" or r["conclusion"] == "success"
                    for r in existing),
                "existing unsuccessful same-head run requires diagnosis")
        return
    require(os.environ["GITHUB_RUN_ATTEMPT"] == "1",
            "controller rerun cannot POST an absent dispatch")
    record("intent", workflow=number, path=path, sha256=digest,
           payload={"ref": REF})
    uncertain = False
    try:
    ...
    require(len(observed) == 1,
            "missing or ambiguous dispatch; no automatic POST retry permitted")
...
def main():
    ...
    require([p["sha"] for p in request("/git/commits/" + controller)["parents"]] == [BASE],
            "controller must be direct child of pinned dev")
    record("controller-start", candidate=HEAD, tree=TREE, base=BASE, pr=PR,
           parent=PARENT, controller=controller, attempt=os.environ["GITHUB_RUN_ATTEMPT"])

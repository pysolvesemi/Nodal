"""Allowlisted targeted dispatch for the Increment 42 formatting-repair successor."""
...
HEAD = '17394368c3ab57bcf88e7b551407690754a4b166'
TREE = '784e8b13c64337cbaeb89b771c0da61370cf4573'
FAILED_RUN = 35815121116
FAILED_HEAD = '0bff9004582df2300a040db96808b61c9382d02e'
WORKFLOWS = (
...
def request(path: str, payload: dict | None = None):
    ...
    req = urllib.request.Request(API + path, data=data, headers={
        ...
        'User-Agent': 'Nodal-42-PR134-173943-targeted-v2',
...
def validate_failure():
    run = request(f'/actions/runs/{FAILED_RUN}')
    assert run['workflow_id'] == 338626156
    assert run['head_sha'] == FAILED_HEAD and run['head_branch'] == REF
    assert run['event'] == 'workflow_dispatch' and run['conclusion'] == 'failure'
    jobs = request(f'/actions/runs/{FAILED_RUN}/jobs?per_page=100')['jobs']
    results = {job['name']: job['conclusion'] for job in jobs}
    assert results.get('contracts') == 'failure'
    assert results.get('native') == 'success' and results.get('scala') == 'success'
    assert results.get('required') == 'failure'
...
def main():
    ...
    assert os.environ['GITHUB_REF'] == 'refs/heads/ci-control/nodal-42-pr134-173943-targeted-v2'
    record('controller-start', candidate=HEAD, tree=TREE, base=BASE, pr=PR,
           failed_run=FAILED_RUN, controller=os.environ['GITHUB_SHA'],

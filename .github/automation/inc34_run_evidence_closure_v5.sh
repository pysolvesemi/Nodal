...
patched_runner=${RUNNER_TEMP}/inc34_run_evidence_closure_v5_patched.sh
...
python3 - "${source_runner}" "${patched_runner}" \
  "${followup1}" "${followup2}" "${followup3}" "${followup4}" <<'PY'
...
if anchor not in text:
    raise SystemExit("closure runner v5 injection anchor was not found")

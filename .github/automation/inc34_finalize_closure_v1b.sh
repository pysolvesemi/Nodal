...
for _ in $(seq 1 450); do
  ...
  state=$(latest_materializer_state)
  case "${state}" in
    ...
    missing|completed:failure|completed:cancelled|completed:timed_out|completed:action_required|completed:*)
      echo "materializer state ${state}; running one guarded restart"
      ...
      bash "${controller}/.github/automation/inc34_run_evidence_closure_v1.sh" \
        "${controller}" \
        "${report}/materializer-work" \

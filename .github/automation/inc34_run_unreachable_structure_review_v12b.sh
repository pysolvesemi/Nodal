...
if [[ ${feature_base} != "${remote_feature}" ]]; then
  echo 'feature branch moved before fresh-review hardening' >&2
  exit 1
fi
...
patcher=${controller}/.github/automation/inc34_unreachable_structure_review_v12.py
repairs=${controller}/.github/automation/inc34_unreachable_structure_review_v12_repairs.py
gate patcher-compile python3 -m py_compile "${patcher}" "${repairs}"
if [[ ${failed} -eq 0 ]]; then
  gate apply-fresh-review-hardening python3 "${patcher}" --root "${work}"
  gate apply-controller-repairs python3 "${repairs}" --root "${work}"
fi
...
gate runtime-witness bash -lc \
  "cd '${work}' && output='${RUNNER_TEMP}/increment34-runtime-v12.txt' && ./mill -i examples.continuousTimeApi.runMain nodal.increment34fixture.Increment34RuntimeCheck \"\${output}\" && grep -F 'nested_nonlocal=NODAL-ANALOG-034-014' \"\${output}\""
gate construction-witness bash -lc \
  "cd '${work}' && output='${RUNNER_TEMP}/increment34-construction-v12.txt' && ./mill -i examples.continuousTimeApi.runMain nodal.increment34fixture.Increment34ConstructionCheck \"\${output}\" && grep -F 'public_conditional_snapshots=1' \"\${output}\" && grep -F 'flat_assignments=0' \"\${output}\""
...
if [[ ${failed} -eq 0 ]]; then
  ...
  if ! git -C "${work}" diff --cached --quiet; then
    git -C "${work}" commit -m \
      'fix(increment34): validate unreachable structure and local declarations'
  fi
...
{
  echo '# Increment 34 fresh-review structural hardening v12'
  echo
  echo '```text'
...
result_branch=automation/inc34-unreachable-structure-results-v12
rm -rf result
...
git -C result commit -m \
  "ci(increment34): record fresh-review hardening v12 ${GITHUB_RUN_ID}"
git -C result push --force origin HEAD:"${result_branch}"

if [[ ${failed} -eq 0 ]]; then
  gh pr comment 109 --repo "${GITHUB_REPOSITORY}" \
    --body "Increment 34 fresh review closed the unreachable-structure and nested-declaration-locality gaps. Exact published head: \`${published}\`; run: \`${GITHUB_RUN_ID}\`."
  exit 0
fi

gh pr comment 109 --repo "${GITHUB_REPOSITORY}" \
  --body "Increment 34 fresh-review structural hardening did not publish because a staged gate failed. Logs: branch \`${result_branch}\`, run \`${GITHUB_RUN_ID}\`."
exit 1
